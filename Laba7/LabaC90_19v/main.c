...
int main() {
    ...
    double x,a,x1, x2,step,selection,i,size[1];
    char First[100];
    long int g = 0;
    for (;;) {
        ...
        if (selection == 1) {
            ...
            T_result = fopen("/home/solomonbrokli/LABS/Laba7/LabaC90_19v/T_result.txt", "w");/*записываются данные*/
            for (g = 0; g < 100; g++) {
                fprintf(T_result, "|%lf|%lf|%lf|\n", struct1.siz_G[g], struct1.size_F[g], struct1.size_Y[g]);
            }
            fclose(T_result);
            T_result1 = fopen("/home/solomonbrokli/LABS/Laba7/LabaC90_19v/T_result.txt", "r");/*считываются данные*/
            while (!feof(T_result1)) {
                fscanf(T_result1, "%s", First);
                printf("%s\n", First);
            }
            fclose(T_result1);
        }
